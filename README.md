...
Cloud deployments (Render, etc): see `CLOUD.md`.

### Web Admin Panel (Optional)

If you set `CHATMOCK_ADMIN_PASSWORD`, ChatMock will also expose a small web control panel:
- `http://127.0.0.1:8000/admin` (configure reasoning/web-search flags, persist to `server_settings.json`)
- `http://127.0.0.1:8000/admin/auth` (upload/remove `auth.json`)

This is especially useful for cloud deployments where the `login` flow can’t complete (OAuth callback is localhost-only).
